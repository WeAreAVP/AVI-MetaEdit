...
//---------------------------------------------------------------------------
GUI_Main_xxxx_DateDialog::GUI_Main_xxxx_DateDialog(Core* _C, const std::string &FileName_, const std::string &Field_, QWidget* parent)
: QDialog(parent)
{
    //Internal
    C=_C;
    ...
    Central->addTab(Central_Calendar, (Field=="OriginationDate" || Field=="OriginationTime")?tr("AVI (EBU Tech 3285) Recommandation"):tr("INFO (Microsoft definition) requirements"));
    ...
    //Filling
    string ValueS=C->Get(FileName, Field);
    TextEdit->setPlainText(ValueS.c_str());
    if (C->IsValid(FileName, Field, ValueS)
     ...
     && (ValueS.empty() || QDate::fromString(ValueS.c_str(), Qt::ISODate)!=QDate()) || (Field=="IDIT"))
        OnMenu_Calendar();
    else
    {
    ...
    if (Field=="IDIT")
    {
        ...
        //if (ValueS.empty())
        //    TimeEdit_Activated->setText(QTime::currentTime().toString());
    }
